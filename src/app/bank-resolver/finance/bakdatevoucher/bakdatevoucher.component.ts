...
export class BakdatevoucherComponent implements OnInit {
 ...
 // showAlert = false;
  ...
  New() {
    ...
    // this._voucherDt = this.convertStringToDt(this.sys.lastDt);
    this._voucherTyp = "T";
  ...
  Submit() {
    ...
    {
      console.log(this.reportcriteria.value['fromDate'], this.reportcriteria.controls.fromDate.value)
      debugger;
      this.isLoading = true;
      this.getVoucher(this.reportcriteria.controls.fromDate.value.toISOString(), this.reportcriteria.value['voucherNo']);
      this.isRemove=false;
      this.isSave=false;
    }
  }
...
debugger
        ...
        this.tvdGroupRes = this.groupBy(this.tvdRet, function (item) {
          console.log(item.voucher_dt.substr(0,10));
 ...
 putValue(entry:any,i:any){
    ...
    // try {
      ...
      for (let x = 0; x < this.VoucherF.length; x++) {
       ...
       tvdSave.created_dt=this.sys.CurrentDate
        ...
        tvdSave.voucher_dt = this._voucherDt.toISOString();
        // localStorage.setItem('__unAppVouDT', this._voucherDt.toISOString());
        //new Date(Date.UTC(this._voucherDt.getFullYear(), this._voucherDt.getMonth(), this._voucherDt.getDate(), this._voucherDt.getHours(), this._voucherDt.getMinutes()));
  ...
  public groupBy(array, f) {
    ...
    array.forEach(function (o) {
      ...
      groups[group].push(o);
      console.log(o);
      console.log(o.transaction_type, o.voucher_dt.substr(0,10), o.voucher_id, o.approval_status);
    ...
    return Object.keys(groups).map(function (group) {
      ...
      // console.log(f.transaction_type, f.voucher_dt, f.voucher_id, f.approval_status);
  ...
  private getmAccMaster(): void {
    ...
    this.svc.addUpdDel<any>('Mst/GetAccountMaster', dt).subscribe(
      res => {
        ...
        // this.maccmaster = this.maccmasterRet;
        this.maccmaster = this.maccmasterRet.filter(el => el.acc_cd !== 21101);

         let Index = this.maccmasterRet.findIndex(el => el.acc_cd == 21101);
