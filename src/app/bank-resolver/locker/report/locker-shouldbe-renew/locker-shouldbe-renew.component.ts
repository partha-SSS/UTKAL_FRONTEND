...
@Component({
  selector: 'app-locker-shouldbe-renew',
  templateUrl: './locker-shouldbe-renew.component.html',
  styleUrls: ['./locker-shouldbe-renew.component.css']
})
export class LockerShouldbeRenewComponent implements OnInit,AfterViewInit {
  ...
  displayedColumns: string[] = [ 'agreement_no','locker_id','name','rented_till','cust_cd','phone',];
  ...
  public SubmitReport() {
    var dt={
      ...
      this.svc.addUpdDel('Locker/PopulateLockerShouldBeRenewedRep',dt).subscribe(data=>{console.log(data)
...
downloadexcel(){
  ...
  this.exportAsService.save(this.exportAsConfig, 'Locker_SBRNW').subscribe(() => {
    // save started
    console.log("hello")
  });
}
