...
@Component({
  selector: 'app-cal-dep-post',
  templateUrl: './cal-dep-post.component.html',
  styleUrls: ['./cal-dep-post.component.css'],
  providers:[ExportAsService]
})
export class CalDepPostComponent implements OnInit,AfterViewInit {
  @ViewChild('content', { static: true }) content: TemplateRef<any>;
  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  dataSource = new MatTableDataSource()
  displayedColumns: string[] = ['anx_cd','sub_anx_cd', 'trans_date', 'dep_per', 'qty', 'depreciation', 'balance'];
  ...
  public SubmitReport() {
    if (this.reportcriteria.invalid) {
      ...
      this.svc.addUpdDel('Asset/CalculateDepreciation',dt).subscribe(data=>{console.log(data)
        ...
        this.reportData.forEach(e=>{
          ...
          this.depreciation+=e.depreciation;
  ...
  getTotal(){
    ...
    for(let i=0;i<this.filteredArray.length;i++){
      this.depreciation+=this.filteredArray[i].depreciation;
      this.balance+=this.filteredArray[i].balance;
    }
  }
  CalIntt(){
