...
@Component({
  selector: 'app-dds-agent-trans-report',
  templateUrl: './dds-agent-trans-report.component.html',
  styleUrls: ['./dds-agent-trans-report.component.css']
})
export class DdsAgentTransReportComponent {
...
selectItems1=[
 ...
 onLoadScreen(content) {
  ...
  public SubmitReport() {
    if (this.reportcriteria.invalid) {
      ...
      const fromdate = this.reportcriteria.controls.fromDate.value;
      ...
      debugger
      this.svc.addUpdDel('Deposit/PopulateAgentTransData',dt).subscribe(data=>{console.log(data)
        this.reportData=data
         if(!this.reportData){
          ...
          // this.reportData=data;
          this.groupedData = this.groupByTransactionDate(this.reportData);
        }
 ...
 groupByTransactionDate(data: any[]) {
    const grouped = data.reduce((acc, curr) => {
      let dateGroup = acc.find((g: any) => g.transaction_date === curr.transaction_date);
      if (!dateGroup) {
        dateGroup = { transaction_date: curr.transaction_date, agents: [] };
        acc.push(dateGroup);
      }

      let agentGroup = dateGroup.agents.find((g: any) => g.agent_name === curr.agent_name);
      if (!agentGroup) {
        agentGroup = { agent_name: curr.agent_name, products: [] };
        dateGroup.agents.push(agentGroup);
      }

      let productGroup = agentGroup.products.find((g: any) => g.product_code === curr.product_code);
      if (!productGroup) {
        productGroup = { product_code: curr.product_code, accounts: [], total: 0 };
        agentGroup.products.push(productGroup);
      }

      productGroup.accounts.push({
        account_number: curr.account_number,
        account_holder_name: curr.account_holder_name.trim(),
        deposit_amount: curr.deposit_amount
      });
      productGroup.total += curr.deposit_amount;

      agentGroup.total = agentGroup.products.reduce((sum, prod) => sum + prod.total, 0);
      dateGroup.total = dateGroup.agents.reduce((sum, ag) => sum + ag.total, 0);
  ...
  calcSum() {
    ...
    this.calculateOverallTotal(); // Debug: log only changed accounts
  }
...
debugger
  ...
  groupLoans() {
    const filteredData = this.reportData.filter(transaction => transaction.trans_type === 'D');
    const sortedData = filteredData.sort((a, b) => a.acc_num.localeCompare(b.acc_num));
    
    const grouped = sortedData.reduce((acc: { [key: string]: AggregatedTransaction }, transaction) => {
      const agentName = `AGENT:- ${transaction.agent_name}(${transaction.agent_cd})`;
      ...
      acc[agentName].transactions.push(transaction);
      acc[agentName].total_paid_amt += transaction.paid_amt;
      acc[agentName].total_balance_amt += transaction.balance_amt;
      acc[agentName].total_no_of_acc += 1;
    ...
    if (this.groupedData) {
      ...
      for (let group of this.groupedData) {
        this.tot_acc_no += group.total_no_of_acc;
        this.tot_paid_amt += group.total_paid_amt;
      } 
    }
  ...
  filterData() {
    if (this.searchTerm) {
      this.filteredData = this.groupedData.filter(group =>
        group.agent_name.toLowerCase().includes(this.searchTerm.toLowerCase()) ||
        group.agent_name.split('(')[1].replace(')', '').includes(this.searchTerm)
      );
    } else {
      this.filteredData = this.groupedData;
    }
