...
@Component({
  selector: 'app-recov-reg-acc-type-wise',
  templateUrl: './recov-reg-acc-type-wise.component.html',
  styleUrls: ['./recov-reg-acc-type-wise.component.css']
})
export class RecovRegAccTypeWiseComponent implements OnInit{
  ...
  displayedColumns: string[] = ['acc_name','trans_dt','loan_id','cust_name','curr_prn_recov','ovd_prn_recov','curr_intt_recov','ovd_intt_recov','recov_amt','last_intt_calc_dt'];
  modalRef: BsModalRef;
  isOpenFromDp = false;
  isOpenToDp = false;
...
selectItems1=[
  ...
  ngOnInit(): void {
    ...
    this.reportcriteria = this.formBuilder.group({
      ...
      fundType:['O', Validators.required]
    });
 ...
 onLoadScreen(content) {
  ...
  public SubmitReport() {
    if (this.reportcriteria.invalid) {
      ...
      debugger
      this.svc.addUpdDel('Loan/PopulateRecoveryRegisterFundwiseBlockwise',dt).subscribe(data=>{console.log(data)
        // this.reportData=data
         if(!data){
          ...
          this.reportData=data;
          this.reportData2 = this.reportData.reduce((acc, item) => {
            return acc.concat(item.gmloantrans);
          }, []);

          console.log(this.reportData2);

        this.dataSource.data=this.reportData2;
        this.totCount=this.reportData2.length
  ...
  showFirstGroup(){
    ...
    switch(this.selectedValue1){
      ...
      for(let i=0;i<this.reportData2.length;i++){
        this.firstGroup[i]=this.reportData2[i].acc_name
     }
     break;
      case "Party Name":
        for(let i=0;i<this.reportData2.length;i++){
          this.firstGroup[i]=this.reportData2[i].cust_name
       }
    // this.filteredArray=this.reportData2.filter(e=>e.party_name?.toLowerCase().includes(filterValue.toLowerCase())==true)
     break;
       case "Loan ID":
        for(let i=0;i<this.reportData2.length;i++){
          this.firstGroup[i]=this.reportData2[i].loan_id
       }
        // this.filteredArray=this.reportData2.filter(e=>e.loan_id?.toLowerCase().includes(filterValue.toLowerCase())==true)
         break;
  ...
  searchFirstGroup(){
    ...
    switch(this.selectedValue1){
      ...
      case "Loan Type":
      this.filteredArray=this.reportData2.filter(e=>e.acc_name?.toLowerCase().includes(this.bName.toLowerCase())==true)
      break;

      case "Party Name":
    this.filteredArray=this.reportData2.filter(e=>e.cust_name?.toLowerCase().includes(this.bName.toLowerCase())==true)
     break;
      case "Loan ID":
        this.filteredArray=this.reportData2.filter(e=>e.loan_id?.toLowerCase().includes(this.bName.toLowerCase())==true)
         break;
...
debugger
  ...
  getTotal(){
    ...
    for(let i=0;i<this.filteredArray.length;i++){
      this.totIssueSum+=this.filteredArray[i].recov_amt
