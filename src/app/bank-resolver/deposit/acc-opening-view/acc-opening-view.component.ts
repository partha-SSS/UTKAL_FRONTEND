...
@Component({
  selector: 'app-acc-opening-view',
  templateUrl: './acc-opening-view.component.html',
  styleUrls: ['./acc-opening-view.component.css']
})
export class AccOpeningViewComponent implements OnInit {
  ...
  getDepositScheme(accType) {
    var url;
    if(accType==4){
      url=`Deposit/GetFixedDepositScheme?fixeddepsch_id=${this.tm_deposit.user_acc_num}`
    }
    else if(accType==6){
      url=`Deposit/GetRecurringDepositScheme?recurringdepsch_id=${this.tm_deposit.user_acc_num}`
    }
    else{
      return;
    }
    this.svc.addUpdDel<any>(url, null).subscribe(
        res => {
          debugger
          this.isLoading = false;
          this.schemeList = res;
  ...
  setAgent(){
    this.masterModel.tmdeposit.agent_cd=this.tm_deposit.agent_cd
    debugger
  }
  getCustomerList() {
      ...
      res => {
        ...
        if (this.masterModel === undefined || this.masterModel === null) {
          ...
          if (this.masterModel.tmdeposit.acc_num !== null) {
            ...
            this.getDepositScheme(this.tm_deposit.acc_type_cd);
...
debugger
 ...
 validateSbAccount() {
    ...
    if (this.operationType === 'I') {
      ...
      this.svc.addUpdDel<any>('Deposit/GetCustMinSavingsAccNo', temp_tm_deposit).subscribe(
        res => {
          ...
          this.isLoading = false;
          const x = res;
          this.tm_deposit.cert_no = x.toString();
  ...
  processInstallmentAmount() {
    ...
    temp_gen_param.user_acc_num = this.tm_deposit.user_acc_num;
