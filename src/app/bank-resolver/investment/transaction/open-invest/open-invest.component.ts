...
@Component({
  selector: 'app-open-invest',
  templateUrl: './open-invest.component.html',
  styleUrls: ['./open-invest.component.css']
})
export class OpenInvestComponent implements OnInit {
  td_deftransfer: any;
 ...
 bankData=[]
  ...
  inttChange(){
    this.tm_deposit.mat_val=(+(+this.tm_deposit.prn_amt)+(+this.tm_deposit.intt_amt));
    console.log((+this.tm_deposit.prn_amt)+(+this.tm_deposit.intt_amt));
    console.log( this.tm_deposit.mat_val);
  ...
  onDtChange(opdate){
    ...
    this.tm_deposit.opening_dt=opdate
    console.log(this.tm_deposit.opening_dt);
    this.openDate=opdate
    ...
    if (this.tm_deposit.day === undefined || this.tm_deposit.day === null) {
      ...
      var d = new Date(this.tm_deposit.opening_dt);
      console.log(d);
      var year = d.getFullYear();
      var month = d.getMonth();
      var day = d.getDate();
      var c = new Date(year + this.tm_deposit.year, month+this.tm_deposit.month, day+ this.tm_deposit.day);
      console.log(c);
      this.tm_deposit.mat_dt=c
  ...
  assignModelsFromMasterData() {
    ...
    console.log({"TDDEFTRANS": this.td_deftrans,tm_deposit});
      ...
      if (this.tm_deposit.acc_type_cd === null || this.tm_deposit.acc_type_cd === undefined) {
        ...
        this.svc.addUpdDel<any>('INVESTMENT/GetInvOpeningData', this.tm_deposit).subscribe(
          res => {
            ...
            else if(this.masterModel.tmdepositInv.approval_status=='A'){
              this.HandleMessage(true, MessageType.Warning, 'Account was already approved!');
              this.tm_deposit.acc_num = null;
              return;
            }
            else {
...
debugger
    ...
    if (this.td_deftrans.trf_type === 'T') {
      ...
      if (this.tm_deposit.acc_type_cd === 25) {
...
debugger
      if (this.td_deftranstrfList[0].cust_acc_type === undefined ||
        ...
        this.td_deftrans.acc_num =this.td_deftranstrfList[0].cust_acc_number ;
        console.log(this.td_deftrans);
    ...
    console.log(this.masterModel.tmdepositInv.acc_num ,this.td_deftrans.acc_num);
...
//     );
  ...
  InsertAccountOpenData() {
    ...
    this.masterModel.tmtransfer[0].approval_status = 'U';
    this.masterModel.tddeftranstrf[0].brn_cd = this.tm_deposit.brn_cd;
    this.masterModel.tddeftranstrf[0].acc_num =this.td_deftrans.acc_num
    this.masterModel.tddeftranstrf[0].acc_type_cd =this.td_deftranstrfList[0].cust_acc_type;
    this.masterModel.tddeftranstrf[0].trans_cd = this.td_deftrans.trans_cd;
    this.masterModel.tddeftranstrf[0].trans_dt = this.sys.CurrentDate;
    this.masterModel.tddeftranstrf[0].ardb_cd=this.sys.ardbCD;
    this.masterModel.tddeftranstrf[0].approval_status = 'U';
    this.masterModel.tddeftrans.brn_cd = this.branchCode;
    this.masterModel.tddeftrans.trans_dt = this.sys.CurrentDate;
    this.masterModel.tddeftrans.acc_type_cd = this.tm_deposit.acc_type_cd;
    this.masterModel.tddeftrans.acc_num = this.masterModel.tmdepositInv.acc_num;
    this.masterModel.tddeftrans.approval_status = 'U';
    // this.masterModel.tddeftrans.amount = this.masterModel.tmdepositInv.prn_amt;
    ...
    console.log(this.masterModel);
      ...
      this.svc.addUpdDel<any>('INVESTMENT/InsertInvOpeningData', this.masterModel).subscribe(
        ...
        err => {
          ...
          if (this.operationType === 'I') {
            this.masterModel.tmdepositInv.acc_num = null;
          }
    ...
    else // Modify the Account opening Data
    {
      console.log(this.masterModel);
    this.masterModel.tddeftrans.acc_num = this.masterModel.tmdepositInv.acc_num;
      console.log(this.masterModel.tddeftrans.acc_num);
...
debugger
  ...
  setAccountType(accType: number) {
   ...
   console.log(accType)
    ...
    if ((this.tm_deposit.acc_type_cd==22)||(this.tm_deposit.acc_type_cd==23)||(this.tm_deposit.acc_type_cd==24)) {
      ...
      this.DisTrfTyp=true;
      console.log(this.intTransferType);
      
      // this.setTmDepositModel();
    }
    else{
     
      this.DisTrfTyp=false;
 ...
 SetInttTfrType(_opern_Type:any,_acc_type_cd:any){
  ...
  processYearMonthDay() {
    ...
    console.log(this.tm_deposit.opening_dt);
    // this.tm_deposit.mat_dt = this.openDate;
    console.log( this.tm_deposit.mat_dt);
    ...
    this.tm_deposit.mat_dt=c
    this.processInterest();
    // this.tm_deposit.mat_dt.setFullYear(this.tm_deposit.opening_dt.getFullYear() + this.tm_deposit.year);
    // this.tm_deposit.mat_dt.setMonth(this.tm_deposit.opening_dt.getMonth() + this.tm_deposit.month);
    // this.tm_deposit.mat_dt.setDate(this.tm_deposit.opening_dt.getDate() + this.tm_deposit.day);
    
    
    // if (this.operationType === 'I' && this.tm_deposit.acc_type_cd === 23) {
    ...
    // temp_gen_param.adt_temp_dt = this.tm_deposit.opening_dt;
    // temp_gen_param.as_intt_type = this.tm_deposit.intt_trf_type;
    // console.log(temp_gen_param.adt_temp_dt);
    // console.log(typeof (this.tm_deposit.opening_dt.toString()));
    // let convdt=this.tm_deposit.opening_dt.toString()
    // console.log(convdt);
    // debugger

    
    // tslint:disable-next-line: max-line-length
    // if (typeof (convdt) === 'string') {
    //   console.log(convdt);
    //   const date = new Date(convdt);
    //   debugger
    //   this.tm_deposit.opening_dt = this.tm_deposit.opening_dt;
    //   console.log( this.tm_deposit.opening_dt);
    ...
    // console.log(Date.UTC(this.tm_deposit.mat_dt.getFullYear(),
    // this.tm_deposit.mat_dt.getMonth(), this.tm_deposit.mat_dt.getDate()));
    
    // console.log(Date.UTC(this.tm_deposit.opening_dt.getFullYear(), this.tm_deposit.opening_dt.getMonth(),
    // this.tm_deposit.opening_dt.getDate()));
