...
export class PassBookFastPageComponent implements OnInit {
  ...
  calINTT(){
    var dt={
      ...
      "user_acc_num":this.masterModel?.tmdeposit?.user_acc_num
  ...
  loadFastPageData(){
    ...
    this.svc.addUpdDel('Deposit/getAccountOpeningData',dt).subscribe(data=>{
     ...
     if(data){
      ...
      debugger
      if(this.masterModel?.tdnominee.length>0 ||this.masterModel?.tdaccholder.length>0){
      ...
      if(this.accType=="6"){
        ...
        // this.masterModel?.td_nomineeList
