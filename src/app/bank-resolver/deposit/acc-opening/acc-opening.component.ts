...
export class AccOpeningComponent implements OnInit {
  ...
  initializeModels() {
    ...
    this.tm_deposit.user_acc_num = 'CUSTOM';
  ...
  public suggestCustomer2(): Observable<mm_customer> {
    ...
    if (this.tm_deposit.cert_no.length > 0) {
      ...
      prm.as_cust_name = this.tm_deposit.cert_no.toLowerCase();
      ...
      this.svc.addUpdDel<any>('Deposit/GetAccDtls', prm).subscribe(
        res => {
          console.log(res)
          this.isLoading = false;
          console.log(this.tm_deposit.cert_no)
          if (undefined !== res && null !== res && res.length > 0 && res != '' && this.tm_deposit.cert_no) {
  ...
  public SelectCustomer(cust: any): void {
    ...
    console.log(cust)
    this.tm_deposit.cert_no=cust.acc_num
  ...
  saveData() {
    if(this.tm_deposit.prn_amt){
      ...
      if(this.tm_deposit.cert_no == undefined || this.tm_deposit.cert_no == null){
  ...
  validateData() {
    ...
    if ((this.tm_deposit.acc_type_cd === 2 || this.tm_deposit.acc_type_cd === 5 || this.tm_deposit.acc_type_cd === 6)
      ...
      && (this.tm_deposit.cert_no === undefined || this.tm_deposit.cert_no === null || this.tm_deposit.cert_no === "")) {
    ...
    if ((this.operationType === 'I') && (this.tm_deposit.cert_no === undefined || this.tm_deposit.cert_no === null || this.tm_deposit.cert_no === "")
  ...
  InsertAccountOpenData() {
    let ret = -1;
    this.validateData();
    this.masterModel.tmdeposit.user_acc_num=this.tm_deposit.acc_type_cd=='4'?'43':'10'
  ...
  setAccountType(accType: number) {
   ...
   console.log(accType)
    ...
    //PARTHAxxxxx
    // if(this.sys.ardbCD=="4" && accType==9){
    // this.accountTypeDiv = 11;
    // }
    // else{
    ...
    this.setConstitutionType(1);

      this.tm_deposit.month = this.savingsDepoSpclPeriod;
      this.tm_deposit.intt_rt=Number(localStorage.getItem('DDS_INT_RT'))
      ...
      this.setConstitutionType(this.selectedConstitutionList[0]?.constitution_cd);

    }
    //PARTHAxxxxxxxx
    // if (this.operationType === 'I' && this.sys.ardbCD=="4"&& this.tm_deposit.acc_type_cd === 9) {
    //   this.tm_deposit.month = this.savingsDepoSpclPeriod;
 ...
 SetInttTfrType(_opern_Type:any,_acc_type_cd:any){
  ...
  getDepositData(){
    var dt={
      ...
      "cust_cd":this.tm_deposit.cust_cd
    }
    this.isLoading=true;
    this.svc.addUpdDel<any>('UCIC/GetDepositDtls',dt).subscribe(data=>{console.log(data)
...
if(accType=='1'){
      ...
      if(this.filterSavingACC){
        ...
        this.HandleMessage(true, MessageType.Warning, 'No Saving Account Found in Transfer Details for this Customer');
  ...
  validateSbAccount() {
    ...
    if (this.tm_deposit.cert_no === undefined
      || this.tm_deposit.cert_no === null
      || this.tm_deposit.cert_no === "") {
      return;
    }
    ...
    const temp_deposit = new tm_deposit();
    temp_deposit.brn_cd = this.branchCode;
    temp_deposit.acc_num = this.tm_deposit.cert_no;
  ...
  processInstallmentAmount() {
    ...
    temp_gen_param.user_acc_num='10';
  ...
  processPrincipal() {
    ...
    if ((this.tm_deposit.acc_type_cd !== 1) && (this.tm_deposit.acc_type_cd !== 7) && (this.tm_deposit.acc_type_cd !== 13)) {
      ...
      temp_gen_param.user_acc_num = '43'
  ...
  processYearMonthDay() {
    ...
    if (this.operationType === 'I' && (this.tm_deposit.acc_type_cd === 4)||(this.tm_deposit.acc_type_cd === 2)||(this.tm_deposit.acc_type_cd === 5)||(this.tm_deposit.acc_type_cd === 6)) {
      ...
      this.calculateInterestRate(this.tm_deposit.acc_type_cd);
    }
    if (this.operationType === 'I' && (this.tm_deposit.acc_type_cd === 11)) {
        this.tm_deposit.intt_rt=Number(localStorage.getItem('DDS_INT_RT'))
    }
  ...
  calculateInterestRate(acc_type_cd) {
    const temp_gen_param2 = new p_gen_param();
    temp_gen_param2.ardb_cd=this.sys.ardbCD;
    temp_gen_param2.acc_cd = acc_type_cd==4?2:acc_type_cd;
    temp_gen_param2.from_dt = this.sys.CurrentDate;
    temp_gen_param2.ls_catg_cd = this.tm_deposit.category_cd;
    temp_gen_param2.ai_period = Math.floor((Date.UTC(this.tm_deposit.mat_dt.getFullYear(),
      this.tm_deposit.mat_dt.getMonth(), this.tm_deposit.mat_dt.getDate()) -
  ...
  getShareNomine(ucic:Number) {
    ...
    this.svc.addUpdDel<any>(`UCIC/GetTdMemNomine?member_id=${cust_cd}`,null).subscribe(
      ...
      debugger
        this.isLoading = false;
        if (res) {
          const nomineeData=res;
          this.masterModel.tdnominee = nomineeData.map(nominee => {
            const temp_td_nominee = new td_nominee();
            temp_td_nominee.brn_cd= undefined;
            temp_td_nominee.acc_type_cd= undefined;
            temp_td_nominee.acc_num = undefined;   // Assuming member_id maps to acc_num
            temp_td_nominee.nom_id = Number(nominee.nomine_id);
            temp_td_nominee.nom_name = nominee.nomine_name;
            temp_td_nominee.nom_addr1 = nominee.nomine_addr;
            temp_td_nominee.nom_addr2 = ""; // Assuming a second address is not available
            temp_td_nominee.phone_no = nominee.nomine_phone;
            temp_td_nominee.percentage = nominee.nomine_perc;
            temp_td_nominee.relation = nominee.nomine_reln;
          
            return temp_td_nominee;
          });
          this.td_nomineeList=this.masterModel.tdnominee
        console.log(res,this.td_nomineeList,this.masterModel.tdnominee);  
          debugger
        }
        else{
