...
@Component({
  selector: 'app-open-borrowing',
  templateUrl: './open-borrowing.component.html',
  styleUrls: ['./open-borrowing.component.css']
})
export class OpenBorrowingComponent implements OnInit {
...
@ViewChild('callDelete', { static: true }) callDelete: TemplateRef<any>;
...
GetLoanAccountNumberAndInsertData() {
  ...
  this.svc.addUpdDel<any>('Borrowing/PopulateBorrowingAccountNumber', dt).subscribe(
    res => {
      if(res>0){
        this.tm_loan_all.loan_id=res;
        debugger
        if(+this.tm_loan_all.loan_id>0){
        ...
        this.HandleMessage(true, MessageType.Error, 'Borrowing Account No Creation Failed !!');
      }
    })
...
private GetUnapprovedDepTrans(): void {
  ...
  this.svc.addUpdDel<any>('Common/GetUnapprovedDepTrans', this.td_deftrans).subscribe(
    res => {
      console.log(res);
...
{ 
  this.allUnApproveData
    ...
    if(this.filterData==undefined || this.filterData==null || this.filterData.del_flag=="Y"){
      this.HandleMessage(true, MessageType.Warning, 'No record found for this BORROWING!!')
      return;
    }
...
else if(e.target.value.length>0 && this.operationType == 'N')
{
  return
}
else{
...
getBorrowingCall(){
  debugger
this.svc.addUpdDel<any>('Borrowing/GetBorrowingOpeningData', this.tm_loan_all).subscribe(
res => {
...
setTdDepTrans(){
  this.td_deftrans.instl_start_dt=this.tm_loan_all.instl_start_dt;
  this.td_deftrans.instl_no=this.tm_loan_all.instl_no;
  this.td_deftrans.curr_intt_rate=this.tm_loan_all.curr_intt_rate;
  this.td_deftrans.ovd_intt_rate=this.tm_loan_all.ovd_intt_rate;
...
DeleteClick(){
  this.modalRef.hide();
  this.isLoading = true;
  this.svc.addUpdDel<any>('Borrowing/DeleteBorrowingOpeningData', this.td_deftrans).subscribe(
  res => {
    if(res==0){
      this.HandleMessage(true, MessageType.Sucess, 'Borrowing sucessfully deleted !!');
      // this.td_deftrans=null;
      // this.tm_loan_all=null;
      // this.td_deftranstrfList=null
    }
    else{
      this.HandleMessage(true, MessageType.Error, 'Borrowing not deleted  !!');
    }
    this.isLoading=false
    debugger
  },
