...
@Component({
  selector: 'app-npa-all-with-group',
  templateUrl: './npa-all-with-group.component.html',
  styleUrls: ['./npa-all-with-group.component.css']
})
export class NpaAllWithGroupComponent implements OnInit {
  ...
  displayedColumns: string[] = ['block_name','service_area_name','vill_name','activity','acc_desc','loan_id','party_name','disb_dt','disb_amt', 'prn_due', 'intt_due','npa_dt','ovd_prn','ovd_intt','penal_intt','stan_prn','substan_prn','d1_prn','d2_prn','d3_prn','totalNPA','provision'];
  modalRef: BsModalRef;
  isOpenFromDp = false;
  isOpenToDp = false;
 ...
 onLoadScreen(content) {
  ...
  public SubmitReport() {
    if (this.reportcriteria.invalid) {
      ...
      this.svc.addUpdDel('Loan/NPAListNew',dt).subscribe(data=>{console.log(data)
         ...
         if(!data){
          ...
          let villxxx = '';
            for (let i = 0; i < this.reportData.length; i++) {
              if(this.reportData[i].loan_id==null){
                this.reportData[i].npa_dt=null
                this.reportData[i].disb_dt=null
                this.reportData[i].recov_dt=null
                this.reportData[i].npa_dt=null
                this.reportData[i].acc_cd=9999 
              }
              if(this.reportData[i].npa_dt=='01/01/0001 00:00'){
                this.reportData[i].npa_dt=null
              }
